...
# Drawing the numbers and deleting the boards
# that have won. Stopping when boards is empty
del_boards = []
for drawn_number in numbers:
    ...
    for iboard in range(len(boards)):
        ...
        if win == True:
            ...
            del_boards.append(iboard)
            win = False
    for db in del_boards:
        boards[db] = "RM"
    del_boards = []
    boards = list(filter(("RM").__ne__, boards))

# Getting sum of winning board + answer
sum_win_board = 0
